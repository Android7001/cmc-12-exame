...
def WalkinRobot():
    ...
    for i in range(steps):
        ...
        if (Vy >= 0 and balanco_esquerda) or (Vy < 0 and not balanco_esquerda):
            ...
            pe_esq_delta_x = pe_esq_x_temp * math.cos(pe_esq_psi_temp[-1]) - pe_esq_y_temp * math.sin(pe_esq_psi_temp[-1])
            pe_esq_delta_y = pe_esq_x_temp * math.sin(pe_esq_psi_temp[-1]) + pe_esq_y_temp * math.cos(pe_esq_psi_temp[-1])
    ...
    return tempo, xcm, ycm, pe_esq_x, pe_esq_y, pe_esq_z, pe_esq_psi, pe_dir_x, pe_dir_y, pe_dir_z, pe_dir_psi

# Simulação do caminhar do robô
tempo, xcm, ycm, pe_esq_x, pe_esq_y, pe_esq_z, pe_esq_psi, pe_dir_x, pe_dir_y, pe_dir_z, pe_dir_psi = WalkinRobot()
...
axs[0].set_ylabel('Posição (cm)', fontsize=10)
axs[0].set_title('Posições do Centro de Massa (CM)', fontsize=10)
axs[0].legend()
axs[0].grid(True)
...
axs[1].set_ylabel('Posição (cm)', fontsize=10)
axs[1].set_title('Posições do Pé Esquerdo', fontsize=10)
axs[1].legend()
axs[1].grid(True)
...
axs[2].set_ylabel('Posição (cm)', fontsize=10)
axs[2].set_title('Posições do Pé Direito', fontsize=10)
axs[2].legend()
axs[2].grid(True)
