...
def WalkinRobot():
    ...
    for i in range(steps):
        ...
        if (Vy >= 0 and balanco_esquerda) or (Vy < 0 and not balanco_esquerda):
            ...
            pe_esq_x_temp, pe_esq_y_temp, pe_esq_z_temp, pe_esq_psi = pe_esq.get_foot_position([Vx, Vy, Vphi], pe_esq_pos_inicial)
            pe_esq_pos_inicial = [pe_esq_x_temp[-1], pe_esq_y_temp[-1], pe_esq_psi[-1]]  # atualização da posição inicial para o próximo passo

            pe_esq_delta_x = pe_esq_x_temp * np.cos(pe_esq_psi) - pe_esq_y_temp * np.sin(pe_esq_psi)
            pe_esq_delta_y = pe_esq_x_temp * np.sin(pe_esq_psi) + pe_esq_y_temp * np.cos(pe_esq_psi)
            ...
            pe_dir_x_temp, pe_dir_y_temp, pe_dir_z_temp, pe_dir_psi = pe_dir.get_foot_position([Vx, Vy, Vphi], pe_dir_pos_inicial)
            pe_dir_pos_inicial = [pe_dir_x_temp[-1], pe_dir_y_temp[-1], pe_dir_psi[-1]]  # Atualização da posição inicial do pé direito pro próximo passo

            pe_dir_delta_x = pe_dir_x_temp * np.cos(pe_dir_psi) - pe_dir_y_temp * np.sin(pe_dir_psi)
            pe_dir_delta_y = pe_dir_x_temp * np.sin(pe_dir_psi) + pe_dir_y_temp * np.cos(pe_dir_psi)
